relax.lm <- function(mesh1,lm1,mesh2,lm2,lm,ray=T,tol=NULL,split=1000,iter=1,rhotol=0.7,sdmax=3,quant.be=0.95,tol2=NULL)
  {
    mesh1 <- warp.mesh(mesh1,lm1,lm2)
    lm <- tps3d(lm,lm1,lm2)
    ...
    if (ray) ## check if search along normals is selected
    ...
    else
      {
        ...
        if (!is.null(lm))
          {lmini <- projRead(lm,mesh1)  ### get normals from additional landmarks
...
#### unify mesh using the newly aquired corresponding points ###
   ...
   # w.mesh <- unify.mesh(mesh1,mesh2,vb.m1,dat,ray=ray,tol=tol2)
    ...
    return(list(norm=norm,dat=dat,dist1=dist1,rho=rho,dataslide=dataslide,rhoex=rhoex,dif.be=dif.be))
  }
