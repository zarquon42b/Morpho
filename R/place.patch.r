place.patch <- function(dat.array,path,atlas.mesh,atlas.lm,patch,which.fix,prefix="skull_",tol=5,ray=T,outlines=NULL,SMvector=NULL,deselect=TRUE,inflate=NULL)
  {
    ...
    for(i in 1:n){
      ...
      if (!is.null(inflate)) ### use for mullitlayer meshes to avoid projection inside
        {
              atlas.warp <- warp.mesh(atlas.mesh,atlas.lm,slide)
              tps.lm <- proj.read(tps.lm,atlas.warp,readnormals=TRUE)
              tps.lm$vb[1:3,] <- tps.lm$vb[1:3,]+inflate*tps.lm$normals[1:3,]
              tps.lm <- ray2mesh(tps.lm,tmp.name,inbound=TRUE,tol=tol)
