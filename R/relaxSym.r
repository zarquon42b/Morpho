...
relaxSymtest <- function(mesh1,mesh2,sigma,gamma=2,W0,f,oneway=F,k=1,nh=NULL,append=F,...)
{
  sigma0 <- sigma
  M0 <- t(mesh2$vb[1:3,])
  S0 <- t(mesh1$vb[1:3,])
 ...
 # M <-  t(ray2mesh(mesh2,mesh1,angmax=0.7,tol=15)$vb[1:3,])
  if(append)
    mesh1tmp <- mesh1
    {
      mesh1tmp$vb <- mesh1$vb[,1 :(dim(mesh1$vb)[2]-dim(W0)[1])]
   ...
   if (oneway)
    ...
    {
      M <-  t(closemeshKD(mesh2,mesh1tmp,sign=F)$vb[1:3,])
    }
      if (!is.null (nh))
    {
      clostIndW <- nn2(S,W0,k=nh,...)$nn.idx
      clostIndP <- nn2(M,W0,k=nh,...)$nn.idx
    }
  D1 <- S-S0
  D2 <- M-M0
