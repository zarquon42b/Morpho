...
#' @param minscalef double: for types "similarity" and "affine", minimum scale factor per iteration, prevents excessive shrinking in first iteration.
#' @param mincumscalef double: for types "similarity" and "affine", minimum cumulative scale factor, prevents shrinking below threshold (e.g. 0.7), values above 1 will enforce size increase, should only be used if both landmark configurations are in the same scale.
...
#' @export
icpmat = function (x,y,iterations,mindist=1e+15,subsample=NULL,type=c("rigid","similarity","affine"),weights = NULL,threads=1,centerweight=FALSE,minscalef=0.9,mincumscalef=NULL) 
{
  m <- ncol(x)
  if (m == 2) {
    x <- cbind(x,0)
  ...
  if (!is.null(subsample)) {
    ...
    subs <- fastKmeans(x, k = subsample, iter.max = 100, 
