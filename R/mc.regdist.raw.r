mc.regdist.raw<-function(dataarray,plot=TRUE,main="",rho="angle",dist.mat.out=FALSE,reflect=FALSE)
{     proc<-mc.procGPA(dataarray,CSinit=TRUE)
      x<-proc$rotated
      n<-dim(x)[3]
      m<-dim(x)[2]
      k<-dim(x)[1]
      ...
      procdis<-sum(qm^2)/n
        ...
        procdistmat<-matrix(NA,n,n) #calc rho from angle between rotated configs
          for (i in 1:n)
           {for (j in 1:n)
           if (rho=="riemdist"){{procdistmat[i,j]<-riemdist(x[,,i],x[,,j])}}  # riemann dist. from shapes package
